...
describe("Option", () => {
  ...
  describe(".mapOr()", () => {
    it("calls `f` if the Option is Some, returns `def` otherwise", () => {
      const f = jest.fn((x) => x * 2);
  ...
  describe(".mapOrElse()", () => {
    it("calls `def` if the Option is None, calls `f` otherwise", () => {
  ...
  describe(".flatten()", () => {
    it("unwraps a nested Option when is Some", () => {
      const opt = Some(Some(1));
      expect(opt.flatten().unwrap()).toEqual(1);
    });

    it("returns itself when the Option is None", () => {
      const opt = None();
      expect(opt.flatten()).toBe(opt);
    });
  });
});
